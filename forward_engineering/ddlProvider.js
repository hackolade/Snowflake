...
module.exports = (baseProvider, options, app) => {
  ...
  const getOutOfLineConstraints = (
    ...
    const constraints = [...foreignKeyConstraints, ...primaryKeyConstraints, ...uniqueKeyConstraints].map(constraint =>
  ...
  return {
    createSchema({
      ...
      const tagsStatements = tags.map(tag => this.createTag({ tag, schemaName: currentSchemaName, isCaseSensitive }));
      ...
      if (databaseName) {
        statements.push(assignTemplates(templates.createDatabase, { name: getName(isCaseSensitive, databaseName) }));
      }
    ...
    createTable(tableData, isActivated) {
      ...
      const fileFormat = tableData.fileFormat
        ? tab(
            'FILE_FORMAT = ' + getFileFormat(tableData.fileFormat, tableData.formatTypeOptions, tableData.formatName),
            ' ',
          )
        : '';
      ...
      if (tableData.dynamic || tableData.iceberg) {
        ...
        const pattern = preSpace(tableData.externalOptions.PATTERN && 'PATTERN=' + tableData.externalOptions.PATTERN);

        return assignTemplates(templates.createExternalTable, {
          name: tableData.fullName,
          tableOptions: addOptions(
            [partitionKeys, fileFormat, location, refreshOnCreate, autoRefresh, pattern, copyGrants, tagsStatement],
            comment,
          ),
          orReplace,
          tableIfNotExists,
          ...
          ? sequences
              .map(sequence =>
                clean({
                  ...
                  comment: preSpace(
                    sequence.sequenceComments && `COMMENT=${escapeString(scriptFormat, sequence.sequenceComments)}`,
                  ),
                }),
              )
          ...
          ? fileFormats
              .map(fileFormat =>
                clean({
                  ...
                  formatTypeOptions: clean(getFormatTypeOptions(fileFormat.fileFormat, fileFormat.formatTypeOptions)),
                  comment: preSpace(
    ...
    hydrateTable({ tableData, entityData, jsonSchema }) {
        ...
        .reduce((result, column) => {
          ...
          return {
            ...
            [column.primaryKeyConstraintName]: Array.isArray(result[column.primaryKeyConstraintName])
              ? uniqBy(
                  [...result[column.primaryKeyConstraintName], { name: column.name, isActivated: column.isActivated }],
                  'name',
                )
        ...
        .reduce((result, column) => {
          ...
          return {
            ...
            [column.uniqueKeyConstraintName]: Array.isArray(result[column.uniqueKeyConstraintName])
              ? uniqBy(
                  [...result[column.uniqueKeyConstraintName], { name: column.name, isActivated: column.isActivated }],
                  'name',
                )
              : [{ name: column.name, isActivated: column.isActivated }],
          };
      ...
      return {
        ...
        dataRetentionTime: !isNaN(firstTab.DATA_RETENTION_TIME_IN_DAYS) ? firstTab.DATA_RETENTION_TIME_IN_DAYS : '',
        ...
        copyOptions: isEmpty(firstTab.stageCopyOptions) ? {} : clean(getStageCopyOptions(firstTab.stageCopyOptions)),
        cloneTableName: getName(firstTab.isCaseSensitive, get(tableData, `relatedSchemas[${firstTab.clone}].code`, '')),
        likeTableName: getName(firstTab.isCaseSensitive, get(tableData, `relatedSchemas[${firstTab.like}].code`, '')),
